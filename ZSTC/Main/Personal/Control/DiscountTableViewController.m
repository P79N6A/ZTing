...
@interface DiscountTableViewController ()<CYLTableViewPlaceHolderDelegate>

{
    NSMutableArray *_discountData;
...
- (void)_loadData {
    ...
    [params setObject:@"5" forKey:@"pushType"];    // 消息类型(12进出场消息 3账户消息 4资产消息 5优惠消息)
    ...
    [[ZTNetworkClient sharedInstance] POST:msgUrl dict:params progressFloat:nil succeed:^(id responseObject) {
        ...
        if([responseObject[@"success"] boolValue]){
            if(_page == 0){
                [_discountData removeAllObjects];
            }
            ...
            [hisData enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [_discountData addObject:msgModel];
            }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.msgModel = _discountData[indexPath.row];
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    msgDetailVC.msgModel = _discountData[indexPath.row];
